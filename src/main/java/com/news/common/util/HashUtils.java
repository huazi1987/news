...
public abstract class HashUtils {
    ...
    public static String generateUUID() {
        ...
        return toHex(highBites) + toHex(lowBites);
    }
    ...
    public static String hash(String string) {
        try {
            return toHex(hash(string.getBytes("UTF-8")));
    ...
    public static String hashPassword(String string) {
        try {
            ...
            return toHex(hash(value.getBytes("UTF-8")));
        } catch (UnsupportedEncodingException e) {
            LOGGER.error(e.getMessage(), e);
            return null;
        }
    ...
    private static byte[] newHash(MessageDigest digest, File file) {
        ...
        try {
            ...
            digest.update(toBytes(fileLength));
    ...
    public static void main(String[] args) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            String md5 = hash(key);
            System.out.println(String.format("key['%s'] md5['%s'] is %s", key, md5, value.equalsIgnoreCase(md5) ? "OK" : "NO!"));
        }
    }
